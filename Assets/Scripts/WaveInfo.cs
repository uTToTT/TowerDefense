...
public class WaveInfo : MonoBehaviour
{
    ...
    private void EnableWaveInfoFrame(Enemy currEnemy)
    {
        if (_frameWaveInfo != null)
        {
            ...
            if (currEnemy.GetEnemyType() == TypeEnemy.Fast)
            {
                _imageEnemy.sprite = _spriteFastEnemy;
            }
            else if (currEnemy.GetEnemyType() == TypeEnemy.Classic)
            {
                _imageEnemy.sprite = _spriteClassisEnemy;
            }
            else if(currEnemy.GetEnemyType() == TypeEnemy.Armor)
            {
                _imageEnemy.sprite = _spriteArmorEnemy;
            }
            else if(currEnemy.GetEnemyType() == TypeEnemy.Heavy)
            {
                _imageEnemy.sprite = _spriteHeavyEnemy;
            }
            else if(currEnemy.GetEnemyType() == TypeEnemy.King)
            {
